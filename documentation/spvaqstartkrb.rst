...
This section contains a Quick Start Guide (⚡) for Secure PVAccess with `Kerberos
Authenticator`.  It will show you how to configure Secure PVAccess with
a containerised KDC to illustrate how SPVA connects with Kerberos and
exchanges tickets for certificates.
...
^^^^^^^^^^^^^^^^^^^^^^^
    ...
    .. code-block:: shell
        ...
        # Optional: To enable appropriate Authenticator.
