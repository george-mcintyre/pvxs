...
void usage(std::string& executableName, const std::string& initialisationFile) {
	...
	std::cout << "PVXS configurable IOC server\n\n"
	          ...
	          << "\n  (or overriden with the -D option) this command starts an interactive IOC shell, unless the -S flag \n"
	             ...
	             "  -d <path>              Load record-definitions from file.  Each record-definition contains a set of field-\n"
				 "                         definitions:  `record(<type>,\"<name>\") { field(<name>,\"<value>\")... }...`. The \n"
				 "                         definitions specified in the given file are subject to macro substitution\n"
	             "  -G <path>              Load database record-definitions and group-definitions from file.  The file format is identical \n"
				 "                         to the database record-definition file loaded with (-d) except that it \n"
				 "                         additionally accepts group-info-definitions where field-definitions are normally \n"
				 "                         accepted.  group-info-definition := `info(Q:group, {\"<name>\": {<group-field-mapping>...}})`. \n"
				 ...
				 "                         the extension is .db, however these files can't be loaded with the (-d) option.\n"
