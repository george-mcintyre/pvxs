...
PVXS_API int NID_SPvaSoftExpirationDate = NID_undef;
 ...
 *
 * It will also create and register the custom certificate status, config URI, and soft expiration OIDs.
 */
PVXS_API void sslInit() {
    ...
    if (NID_SPvaCertStatusURI == NID_undef) {
        ...
        // Create the custom soft expiration date OID
        NID_SPvaSoftExpirationDate = OBJ_create(NID_SPvaSoftExpirationDateID, SN_SPvaSoftExpirationDate, LN_SPvaSoftExpirationDate);
        if (NID_SPvaSoftExpirationDate == NID_undef) {
            throw std::runtime_error("Failed to create NID for " SN_SPvaSoftExpirationDate ": " LN_SPvaSoftExpirationDate);
        }
    }
}
